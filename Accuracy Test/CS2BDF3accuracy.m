function [u,num_iter] = CSBDF3accuracy2(Spara,t,tau,rho,a,f,u0,S)
% Variable-step convex-splitting BDF3 scheme for Square phase field crystal model;
...
for k=3:N
    ...
    A = a0/tau(k) - (1-epsi)*DLap - (-S*taum^3*a0/tau(k))*DDLap - DDDLap;
    force = f(X,Y,t(k+1));
    if k == 3
        F1 = a0/tau(k)*fft2(u(1:M,1:M,k)) - a1/tau(k-1)*fft2(u(1:M,1:M,k)-u(1:M,1:M,k-1))...
            ...
            + S*taum^3*a0/tau(k)*DDLap.*fft2(u(1:M,1:M,k));
    elseif k == 4
        F1 = a0/tau(k)*fft2(u(1:M,1:M,k)) - a1/tau(k-1)*fft2(u(1:M,1:M,k)-u(1:M,1:M,k-1))...
            - a2/tau(k-2)*fft2(u(1:M,1:M,k-1)-u(1:M,1:M,k-2)) + fft2(force(1:M,1:M))...
            + S*taum^3*a0/tau(k)*DDLap.*fft2(u(1:M,1:M,k))...
            - S*taum^3*a1/tau(k-1)*DDLap.*fft2(u(1:M,1:M,k)-u(1:M,1:M,k-1));
    else
        F1 = a0/tau(k)*fft2(u(1:M,1:M,k)) - a1/tau(k-1)*fft2(u(1:M,1:M,k)-u(1:M,1:M,k-1))...
            - a2/tau(k-2)*fft2(u(1:M,1:M,k-1)-u(1:M,1:M,k-2)) + fft2(force(1:M,1:M))...
            + S*taum^3*a0/tau(k)*DDLap.*fft2(u(1:M,1:M,k))...
            - S*taum^3*a1/tau(k-1)*DDLap.*fft2(u(1:M,1:M,k)-u(1:M,1:M,k-1))...
            - S*taum^3*a2/tau(k-2)*DDLap.*fft2(u(1:M,1:M,k-1)-u(1:M,1:M,k-2));
    end
    k1 = 1 + 1/rho(k);
    k2 = 1 + 1/rho(k) + 1/rho(k)/rho(k-1);
    c1 = k1*k2/(k1-1)/(k2-1);
    c2 = k2/(k1-1)/(k1-k2);
    c3 = -k1/(k1-k2)/(k2-1);
    u_cs = c1*u(1:M,1:M,k) + c2*u(1:M,1:M,k-1) + c3*u(1:M,1:M,k-2);
    F1 = F1 + 2*DDLap.*fft2(u_cs);
